# Round 1 — general
**Date:** 2026-09-02
**Scope reviewed:** branch task/006-002-dev-cli-replace-ps1-scripts vs origin/master (product commit 248a031)

## Summary

Replaces PowerShell build/test/pack/push/hooks with a Nuru `tools/dev-cli` + `bin/dev`, thin `workflow.yml`, and TimeWarp `.githooks`. Task requirements largely land: required commands and capabilities are present, leftover `.ps1` are gone, CI no longer shells `pwsh` or ad-hoc `dotnet nuget push`, and mode detection matches the Design region (`workflow_dispatch` → merge; release/break-glass/probe behave). Dominant residual risk is the transitional `src/` vs `source/` layout: release `check-version` gates on a different Version file than pack emits.
