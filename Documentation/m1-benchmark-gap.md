...
Measured on AMD Ryzen 9 7950X3D, Ubuntu, BenchmarkDotNet 0.13.10, `.NET 10.0.11` host, 8 iterations. Project: `test/TimeWarp.Mediator.Benchmarks.Comparison`. Ping/Pong, no pipeline behaviors.
...
```bash
DOTNET_ROLL_FORWARD=LatestMajor dotnet run --project test/TimeWarp.Mediator.Benchmarks.Comparison -c Release
```
